...
class GenTargets(nn.Module):
    ...
    def forward(self, srcs, gt_boxes, batch_classes):
        ...
        [1]gt_boxes [batch_size,m,4]  FloatTensor  GT boxes
        [2]classes [batch_size,m]  LongTensor      GT classes
        Returns
    ...
    def _gen_level_targets(self, out, gt_boxes, classes, stride, limit_range, sample_radiu_ratio=1.5):
        '''
        Args
        out list contains [[batch_size,class_num,h,w],[batch_size,1,h,w],[batch_size,4,h,w]] 原始的token
...
# Focal Loss 被下面的方法调用，后面两个参数值 论文中有提及
...
def compute_cls_loss(preds, targets, mask):
    '''
    Args
    preds: list contains five level pred [batch_size,class_num,_h,_w]  前景分数
    targets: [batch_size,sum(_h*_w),1]  label
    mask: [batch_size,sum(_h*_w)] 有意义的label mask
    '''
